...
2. **✅ Конфликт PIN‑сценариев (tag/edit) с add flow** — ранее ввод username на шаге добавления мог интерпретироваться как PIN (активный `/tag` или редактирование). Исправлено за счёт дополнительной очистки состояний при входе в add flow (`add_new_callback`, `add_from_check_photo_callback`) и защитных проверок в `tag_pin_input` и `edit_pin_input`, которые завершают устаревшие PIN‑флоу. Дата исправления: 2026‑01‑29.
...
- Перед запуском сценария `add_new_callback` очищает предыдущее состояние через `clear_all_conversation_state` и дополнительно удаляет PIN/tag/edit‑флаги (`pin_attempts`, `tag_manager_name`, `tag_new_tag`, `editing_lead_id`), что предотвращает протекание старых сценариев (например, PIN) в add flow.
...
- При старте add flow через `handle_photo_message` вызывается `clear_all_conversation_state`, что гарантированно завершает предыдущие сценарии (check/edit/tag/add) перед началом нового добавления.
...
- Если пересылка происходит во время другого flow (edit/tag), `handle_forwarded_message` проверяет `current_state` и, при активных edit/tag‑сценариях, отклоняет пересылку с сообщением пользователю, не вмешиваясь в текущий сценарий.

**Потенциальные проблемы**: Нет

---

### 2.8. Предварительное редактирование полей на этапе Review (до сохранения)
**Сценарий**: Пользователь заполнил данные лида → попал на экран Review → нажимает «✏️ Редактировать поля» → меняет одно или несколько полей → возвращается к Review → сохраняет лид.

**Цепочка функций**:
- `show_add_review` (строка 4928) → показывает обзор введённых данных и кнопку «✏️ Редактировать поля»
- `edit_fullname_from_review_callback` (строка 5763) → открывает локальное pre-save меню редактирования без PIN
- `add_edit_field_*_from_review_callback` (строки 5809–5851) → выбирают конкретное поле (`fullname`, `telegram_name`, `telegram_id`, `facebook_link`) для редактирования
- `add_field_input` (строка 4264) → обрабатывает ввод поля, используя флаг `return_to_review` для возврата к обзору
- `show_add_review` → повторно показывает экран Review с обновлёнными данными

**Статус**: ✅ Работает корректно
- Редактирование до сохранения работает в рамках add flow и **не требует PIN‑кода** (в отличие от сценариев редактирования сохранённого лида).
- После изменения любого поля пользователь возвращается на экран Review, а не в начало мастера.
- В состоянии `ADD_REVIEW` ConversationHandler обрабатывает все новые callbacks (`edit_fullname_from_review`, `add_edit_field_fullname`, `add_edit_field_telegram_name`, `add_edit_field_telegram_id`, `add_edit_field_fb_link`, `add_edit_back_to_review`), а `unknown_callback_handler` учитывает их при активации/возобновлении add flow.
...
- Навигация работает
- Перед входом в редактирование пользователь, как правило, завершает другие сценарии с помощью `/q` или `/start`. Дополнительно, запуск `/tag` всегда очищает состояние редактирования через `clear_all_conversation_state`, предотвращая пересечение tag/edit.
...
- Все этапы обрабатываются через ConversationHandler
- Навигация работает
- Команда `/tag` перед установкой `TAG_PIN` всегда вызывает `clear_all_conversation_state` и очищает `user_data_store`, что гарантированно завершает любые активные сценарии (add/edit/check) перед запуском PIN‑флоу для смены тега.
...
При этом:
- `unknown_command_handler` никогда не перехватывает `/start`, `/q`, `/skip`, `/tag` — эти команды всегда обрабатываются своими хэндлерами и корректно завершают текущие сценарии.
- В случае обнаружения стейл‑состояния (наличие `_conversation_`‑ключей без активного сценария) оба обработчика очищают состояние через `clear_all_conversation_state` и возвращают пользователя в главное меню, что считается безопасным завершением предыдущего сценария.

---

### 5.4. Переход из PIN‑сценариев (tag/edit) в add flow
**Сценарий**: Пользователь вводит `/tag` или начинает редактирование лида (PIN), затем возвращается в главное меню и запускает добавление лида (через «Добавить» или фото/фото‑чек).

**Цепочка функций**:
- `tag_command` / `edit_lead_callback` → устанавливают `TAG_PIN` / `EDIT_PIN`
- `tag_pin_input` / `edit_pin_input` → обрабатывают PIN
- `add_new_callback` / `add_from_check_photo_callback` / `handle_photo_message` → запускают add flow

**Статус**: ✅ Исправлено

**Выявленная ранее проблема**:
1. **ПРОБЛЕМА (историческая)**: При незавершённом PIN‑сценарии ввод username на шаге добавления воспринимался как ввод PIN. В результате после шага с username бот запрашивал PIN‑код вместо перехода к следующему шагу ADD flow.
   - **Причина**: Активное состояние `TAG_PIN` или `EDIT_PIN` продолжало существовать в `context.user_data`, а глобальные обработчики PIN перехватывали текстовые сообщения, даже когда пользователь уже начал add flow.

**Текущее решение**:
- `add_new_callback` и `add_from_check_photo_callback` перед запуском add flow:
  - вызывают `clear_all_conversation_state(context, user_id)`;
  - дополнительно удаляют флаги `pin_attempts`, `tag_manager_name`, `tag_new_tag`, `editing_lead_id`.
- В `tag_pin_input` и `edit_pin_input` добавлены защитные проверки:
  - если нет признаков активного tag‑флоу (`tag_manager_name`/`tag_new_tag`) или редактирования (`editing_lead_id`), обработчик считает PIN‑флоу устаревшим, очищает `pin_attempts` и возвращает `ConversationHandler.END`, не перехватывая ввод для других сценариев.

**Потенциальные проблемы**:
- Требуется периодически мониторить логи на предмет вызовов `tag_pin_input`/`edit_pin_input` без активных tag/edit‑флагов, чтобы убедиться, что устаревшие PIN‑флоу действительно корректно завершаются и не пересекаются с add flow.
...
- `unknown_callback_handler` также обрабатывает callbacks в состоянии ADD_REVIEW (строки 2208-2220)
- Все callbacks (`add_save`, `add_back`, `add_cancel`, `edit_fullname_from_review`, `add_edit_field_fullname`, `add_edit_field_telegram_name`, `add_edit_field_telegram_id`, `add_edit_field_fb_link`, `add_edit_back_to_review`) обрабатываются корректно
